...
private:
    ...
    typedef GridStaticPtr				<VertexType, typename MetroMesh::ScalarType >								MetroMeshGridVertex;
...
float Sampling<MetroMesh>::AddSample(const Point3x &p )
{
    ...
    if(Flags & SamplingFlags::USE_AABB_TREE)
      f=tri::GetClosestFaceEP<MetroMesh,MetroMeshAABB>(S2, tS2, p, dist_upper_bound, dist, bestq, normf, ip);
    if(Flags & SamplingFlags::USE_HASH_GRID)
      f=tri::GetClosestFaceEP<MetroMesh,MetroMeshHash>(S2, hS2, p, dist_upper_bound, dist, bestq, normf, ip);
    if(Flags & SamplingFlags::USE_STATIC_GRID)
      f=tri::GetClosestFaceEP<MetroMesh,MetroMeshGrid>(S2, gS2, p, dist_upper_bound, dist, bestq, normf, ip);
    if (Flags & SamplingFlags::USE_OCTREE)
      f=tri::GetClosestFaceEP<MetroMesh,MetroMeshOctree>(S2, oS2, p, dist_upper_bound, dist, bestq, normf, ip);
    if(Flags & SamplingFlags::USE_STATIC_GRID_VERTEX)
      tri::GetClosestVertex<MetroMesh,MetroMeshGridVertex>(S2, gvS2, p, dist_upper_bound, dist, bestq);
...
void Sampling<MetroMesh>::Hausdorff()
{
    ...
    if(Flags & SamplingFlags::USE_STATIC_GRID_VERTEX) gvS2.Set(S2.vert.begin(),S2.vert.end());
    ...
    if(Flags & SamplingFlags::SAVE_ERROR){
      std::pair<float,float> minmax = Stat<MetroMesh>::ComputePerVertexQualityMinMax(S1);
      float minq = minmax.first;
      float maxq = minmax.second;
      vcg::tri::UpdateColor<MetroMesh>::PerVertexQualityRamp(S1, maxq, minq);
    }
