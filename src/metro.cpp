...
void Usage()
{
  printf("\nUsage:  "\
                                        ...
                                        "  -o #       Output basename used in conjuction with -c and/or -h\n"\
                                        "\n"
...
void CreateColorBar(float ColorMin, float ColorMax, const string & name) {

  int scale_width = 20;
  int scale_height = 200;
  img::Image<3, CMesh::ScalarType> img(scale_width, scale_height);
  img.attributes.setRange(0,1);

  for (int i = 0; i < scale_height; ++i) {
    float val  = i/(float)(scale_height-1)*(ColorMax-ColorMin) + ColorMin;
    Color4<CMesh::ScalarType> col;
    col.SetColorRamp(ColorMax,ColorMin,val);
    for (int j = 0; j < scale_width; ++j) {
      img.setValue(j,i,0,col._v[0]);
      img.setValue(j,i,1,col._v[1]);
      img.setValue(j,i,2,col._v[2]);
    }
  }
  bool ok = adjust_range_and_save_RGB(img,name.c_str());
  if(!ok)
    printf("Error saving color scale\n");
}
...
int main(int argc, char**argv)
{
    ...
    string S2NewName;
    if(obase_name.empty()){
      S1NewName=SaveFileName(argv[1]);
      S2NewName=SaveFileName(argv[2]);
    }else{
      S1NewName=obase_name+"_M1-M2.ply";
      S2NewName=obase_name+"_M2-M1.ply";
    }
	...
	// set bounding boxes for S1 and S2 -> already estimated when the ply is imported
    ...
    // set an grid vertex as search structure if input file is a point cloud
    if (S2.fn == 0){
      flags &= ~SamplingFlags::USE_OCTREE & ~SamplingFlags::USE_HASH_GRID & ~SamplingFlags::USE_AABB_TREE & ~SamplingFlags::USE_STATIC_GRID;
      flags |= SamplingFlags::USE_STATIC_GRID_VERTEX;
      ForwardSampling.SetFlags(flags);
    }
    if (S1.fn == 0){
      flags &= ~SamplingFlags::USE_OCTREE & ~SamplingFlags::USE_HASH_GRID & ~SamplingFlags::USE_AABB_TREE & ~SamplingFlags::USE_STATIC_GRID;
  ...
  // print mesh info.
    ...
    printf("Per vertex quality:\n");
    std::pair<float,float> minmax;
    minmax = Stat<CMesh>::ComputePerVertexQualityMinMax(S1);
    float ColorMinS1 = minmax.first;
    float ColorMaxS1 = minmax.second;
    printf("S1->S2: Min \t %f Max \t %f \n", ColorMinS1, ColorMaxS1);

    minmax = Stat<CMesh>::ComputePerVertexQualityMinMax(S2);
    float ColorMinS2 = minmax.first;
    float ColorMaxS2 = minmax.second;
    printf("S2->S1: Min \t %f Max \t %f \n", ColorMinS2, ColorMaxS2);
  ...
  // save error files.
    if(flags & SamplingFlags::SAVE_ERROR)
    {
      ...
      if(ColorMax!=0 || ColorMin != 0){
        vcg::tri::UpdateColor<CMesh>::PerVertexQualityRamp(S1,ColorMax,ColorMin);
        vcg::tri::UpdateColor<CMesh>::PerVertexQualityRamp(S2,ColorMax,ColorMin);
        ColorMinS1 = ColorMinS2 = ColorMin;
        ColorMaxS1 = ColorMaxS2 = ColorMax;
      }
      // create and save color bar
      CreateColorBar(ColorMinS1, ColorMaxS1, obase_name + string("_M1-M2_color_scale.png"));
      CreateColorBar(ColorMinS2, ColorMaxS2, obase_name + string("_M2-M1_color_scale.png"));
      tri::io::ExporterPLY<CMesh>::Save( S1,S1NewName.c_str(),false,p);
      tri::io::ExporterPLY<CMesh>::Save( S2,S2NewName.c_str(),false,p);
    }
    ...
    if(flags & SamplingFlags::HIST) {
      if (obase_name.empty()) {
        ...
        ForwardSampling.GetHist().FileWrite((obase_name + std::string("_M1-M2.csv").c_str()));
        BackwardSampling.GetHist().FileWrite((obase_name + std::string("_M2-M1.csv")).c_str());
      }
    }
